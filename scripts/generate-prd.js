...
// Body paragraph
function P(text) {
  doc.font(R).fontSize(9.5).fillColor("#000").text(text, ML, doc.y, { width: TW, lineGap: 2 });
  gap(0.18);
}
...
doc
  ...
  .fontSize(10.5)
  .fillColor("#333")
  .text("Real Estate Platform  —  Multi-Portal Management Suite", ML, doc.y, {
    width: TW,
    align: "center",
  });
gap(2.8);
...
doc
  ...
  .fontSize(8)
  .fillColor("#777")
  .text("Confidential — Internal Use Only. All rights reserved.", ML, doc.y, {
    width: TW,
    align: "center",
  });
...
for (var i = 0; i < tocItems.length; i++) {
  ...
  if (!isChild) {
    ...
    doc
      ...
      .fontSize(9)
      .fillColor("#000")
      .text(label, ML + 20, doc.y, { width: TW - 20, lineGap: 1.5 });
  }
}
...
P(
  "NxtSft is an end-to-end real estate platform that connects home buyers, property owners, and sales professionals through a unified, role-based portal suite. The platform eliminates brokerage fees for buyers, provides verified property listings across 50+ Indian cities, and equips internal teams with purpose-built tools for lead management, CRM, team supervision, customer support, and platform administration.",
);

P(
  "NxtSft is built on Next.js 15 with React 19 and TypeScript, delivering a fast, server-rendered, fully typed application. The portal system is structured around seven distinct user roles, each with a dedicated interface tailored to that role's specific workflows. All portals are accessible from a single domain and share a common design language and authentication layer.",
);
...
doc
  ...
  .fontSize(10)
  .fillColor("#000")
  .text("Key Platform Objectives", ML, doc.y, { width: TW });
gap(0.2);
...
doc
  ...
  .fontSize(10)
  .fillColor("#000")
  .text("Key Metrics (Platform Goals)", ML, doc.y, { width: TW });
gap(0.2);
...
BUL(
  "ReportsDashboard: shared reports component with filtering and export, embedded in multiple portals",
);

SUB("2.4  Core Domain Concepts");
LBUL(
  "Property",
  "Real estate listing with full specs, images, RERA number, pricing, and owner contact",
);
LBUL(
  "Lead",
  "Prospective buyer captured through the platform, assigned to a sales rep for follow-up",
);
LBUL(
  "Credit / Token",
  "Platform currency used by buyers to unlock owner contact details; purchased via subscription plans",
);
...
P(
  "NxtSft defines seven distinct user roles. Each role maps to a dedicated portal URL and a specific set of capabilities. Role assignment is determined at login and cannot be changed by the user; only an admin or super admin can reassign roles.",
);
...
var roles = [
  [
    ...
    "Super Admin",
    "/sa-portal",
    "Full system access. Manages platform config, all users, billing, AI models, audit logs, and role permissions. No restrictions.",
  ],
  [
    "2",
    "admin",
    "Admin",
    "/admin-portal",
    "Operations manager. Manages team, listings, leads, CRM pipeline, subscriptions, plans, commissions, and marketing. Cannot access super-admin controls.",
  ],
  [
    "3",
    "supervisor",
    "Supervisor",
    "/supervisor-portal",
    "Team lead. Monitors team performance, reassigns leads, reviews escalations, manages visit calendar. Cannot edit plans or billing.",
  ],
  [
    "4",
    "sales",
    "Sales Rep",
    "/sales-portal",
    "Field agent. Manages own leads, logs calls and visits, tracks commission. Scoped to their own lead pool.",
  ],
  [
    "5",
    "support-admin",
    "Support Admin",
    "/support-portal",
    "Customer support agent. Manages support ticket queue, escalations, TAT reporting, and knowledge base.",
  ],
  [
    "6",
    "user",
    "Home Buyer",
    "/user-portal",
    "End consumer. Searches and saves properties, uses credits to unlock contacts, schedules site visits, manages KYC.",
  ],
  [
    "7",
    "customer",
    "Customer",
    "/user-portal",
    "Premium concierge buyer. Same as Home Buyer but designated as a managed/priority customer with Concierge portal label.",
  ],
];
...
gap(0.5);
P(
  "Staff roles (super-admin, admin, supervisor, sales, support-admin) log in via /admin-login. Consumer roles (user, customer) log in via /login. Registration at /register creates a user role session and grants 1 free welcome credit.",
);
...
SUB("4.1  Home Page  ( / )");
P(
  "The public home page is the primary marketing and discovery surface for NxtSft. It is fully server-rendered for SEO and features multiple interactive sections.",
);
...
TAB("City Coverage");
BUL(
  "12 cities: Mumbai, Bengaluru, Delhi NCR, Hyderabad, Pune, Chennai, Kolkata, Ahmedabad, Jaipur, Noida, Gurgaon, Kochi",
);
...
BUL(
  "Validation: valid email format, 10-digit Indian phone number, minimum 6-character password, passwords must match",
);
...
BUL(
  "Property card fields: hero image, title, city / locality, BHK, area (sqft), price, price-per-sqft, match score badge, RERA verified badge, purpose (Sale / Rent)",
);
...
BUL(
  "Status labels: Furnishing (Unfurnished / Semi / Fully), Facing, Floor, Age, Possession date, RERA number",
);
BUL(
  "Amenities grid: up to 12 amenities with icons (e.g., Infinity Pool, Smart Home, EV Charging, Gym)",
);
BUL("Nearby attractions: name + distance in km (schools, hospitals, transit, markets)");
BUL(
  "Owner card: Name, Role (Owner/Agent/Builder), RERA verified badge, Rating, Deals closed, Member since year",
);
...
BUL("Filter: city");
BUL(
  "Agent card fields: avatar, name, city, star rating, total deals closed, active listing count, RERA verified badge",
);
BUL("Click → /agents/[slug] for individual profile");
BUL(
  "Profile page: agent bio, full property listings, contact methods, stats (response time, deals, rating)",
);
...
BUL(
  "Seeker Plans (3 tiers): Instant (1 credit, ₹99), Basic (5 credits, ₹299), Premium (15 credits, ₹699)",
);
BUL(
  "Owner Rental Plans (4 tiers): ranging ₹499 – ₹4,999 with increasing lead counts and listing visibility",
);
...
BUL(
  "Notification preferences: Email Alerts, WhatsApp Notifications, SMS Alerts, Marketing Emails (toggle each)",
);
...
P(
  "The User Portal is the primary interface for registered home buyers and premium customers. It provides 11 navigation sections covering property discovery, credit management, visit scheduling, KYC document management, and preference settings.",
);
...
BUL(
  "Columns: Property name, Locality / City, Date and time viewed, Duration on page, Contact unlocked (Yes / No)",
);
...
P(
  "The Admin Portal is used by regional operations managers to oversee the entire sales funnel, team activity, listing approvals, subscriptions, marketing campaigns, and revenue. It contains 13 navigation tabs.",
);
...
BUL(
  "Team roster table with columns: Name, Email, Role, City, Status (Active / Inactive), Phone, Date Joined, Target Achievement (%)",
);
...
BUL(
  "Columns: Property title, Owner name, Type, City, Submitted date, Status (Pending / Approved / Rejected / In Review)",
);
...
BUL(
  "Leads table with columns: Lead name, Phone, City, Interest (property type), Status (Hot / Warm / Cold / New), Source, Assigned Rep, Deal Value (₹), Last Activity",
);
...
BUL(
  "Kanban-style board with 6 stages: New → Contacted → Qualified → Site Visit → Negotiation → Closed",
);
BUL(
  "Lead cards within each stage: lead name, phone, property interest, deal value, last activity time",
);
...
BUL(
  "Active subscription table: User name, Plan name, Start date, Expiry date, Credits Used / Total, Next Payment date",
);
...
BUL(
  "Table of property view events: Property name, Viewer (user), View duration, Contact unlocked (Yes / No), Timestamp",
);
...
BUL(
  "Builder roster table: developer name, RERA number, active projects count, commission rate (%)",
);
...
SUB("6.11  Reports");
P(
  "Shared ReportsDashboard component. Provides filtering (date range, city, rep, property type) and export (CSV). Report types visible at admin scope:",
);
...
BUL(
  "Fields: name, price, validity (days), max listings, included leads, badge label, feature list",
);
...
P(
  "The Sales Portal is a mobile-optimised field tool for individual sales representatives. All data is scoped to the logged-in rep's own lead pool and performance. It contains 8 navigation tabs.",
);
...
BUL(
  "Entry types: Outbound call, WhatsApp message, Site visit completed, Note added, Lead status changed",
);
...
BUL(
  "Post-call form: outcome (Interested / Not interested / Call back / Wrong number), notes, next action",
);
BUL("Call log: date, lead name, duration, outcome");

SUB("7.5  Site Visits");
BUL(
  "Scheduled visits list: lead name, property address, date, time, status (Confirmed / Pending / Completed / Cancelled)",
);
...
BUL(
  "Commission breakdown table: deal name / property, close date, deal value, commission rate (%), commission amount (₹)",
);
...
P(
  "The Supervisor Portal gives team leads visibility across their assigned team's activity, lead pipeline, and performance metrics. It contains 8 navigation tabs.",
);
...
BUL(
  "Row per team member: name, city, leads open, closed MTD, calls today, current status (Online / Away / Offline)",
);
...
P(
  "The Support Portal is the primary tool for support agents handling customer issues, complaints, and platform queries. It contains 6 navigation tabs.",
);
...
P(
  "The Super Admin Portal provides complete platform control with 15 navigation tabs. It is restricted to the super-admin role and encompasses all administrative functions of the platform.",
);
...
BUL(
  "Columns: Timestamp, Actor (email + role), Action, Entity (user / property / plan), Before value, After value",
);
...
BUL(
  "Feature engineering controls: enable / disable matching signals (location weight, price weight, BHK preference weight, etc.)",
);
...
BUL(
  "Notification templates: pre-built templates for common events (payment reminder, listing approved, etc.)",
);
...
BUL(
  "Create, edit, and delete subscription plans across all plan types (Seeker, Owner Rental, Owner Sell)",
);
BUL(
  "Plan fields: name, price (₹), validity (days), credits included, max listings, leads included, feature list, badge label, popular flag",
);
...
P(
  "NxtSft operates a dual-sided monetisation model: buyers purchase credit packs to unlock owner contact details; property owners purchase listing plans to gain leads and visibility.",
);
...
BUL(
  "Tier 4 — Max:     ₹4,999 · unlimited listings · unlimited leads · priority support + featured badge",
);
...
P(
  "NxtSft uses a React Context-based auth layer (AuthProvider / useAuth hook) backed by localStorage for the demo implementation. In production this would be backed by a secure server-side session or JWT system.",
);
...
LBUL(
  "signIn(role)",
  "Creates a session from ROLE_META for the given role. Grants 3 free credits to user/customer roles if balance is zero.",
);
LBUL(
  "signOut()",
  "Removes the session from localStorage. Credits are intentionally preserved across sign-out.",
);
LBUL(
  "register(name, email, phone)",
  "Creates a user-role session, persists to user registry, grants 1 welcome credit if balance is zero.",
);
LBUL(
  "updateProfile(name, phone)",
  "Mutates the current session name, phone, and derived initials in localStorage.",
);
LBUL("addCredits(n)", "Increments the stored credit balance by n.");
LBUL(
  "useCredit()",
  "Decrements credit balance by 1; returns false if balance is zero (contact unlock gate).",
);
...
for (var pi = 0; pi < total; pi++) {
  ...
  doc
    ...
    .fontSize(8)
    .fillColor("#999")
    .text("NxtSft  —  Product Requirements Document", ML, PH - 48, {
      width: TW / 2,
      align: "left",
    });
  doc
    ...
    .fontSize(8)
    .fillColor("#999")
    .text("Page " + pageNum + " of " + total, ML + TW / 2, PH - 48, {
      width: TW / 2,
      align: "right",
    });
  doc
