...
- [x] Fix (part 3, current): made include globs **root-agnostic** — `["**/.prisma/client/*.so.node", "**/@prisma+client*/**/*.so.node"]` — so they match no matter how Vercel resolves the trace root. Also added `prisma generate` to the web **build command** (`pnpm --filter @nxtsft/db run db:generate && next build`) so the rhel engine is always regenerated, even if Vercel restores a cached install and skips the postinstall. Verified local build: engine in login-route `.nft.json` + generate runs in build. **Redeploy on Vercel WITHOUT build cache.**
- Dead ends ruled out: `queryCompiler` preview (not engine-free with the legacy `prisma-client-js` generator — still demands native engine) and `@prisma/nextjs-monorepo-workaround-plugin` (emits nothing when `@prisma/client` is in `serverExternalPackages`).
