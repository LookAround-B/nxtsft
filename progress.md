...
- [x] **Vercel deploy** *(06-11)* — deploys `apps/web` only (it hosts the site **and** the API via `/api/trpc` + `/api/v1/*`; Fastify `apps/api` is not deployed). Set Vercel **Root Directory = `apps/web`** and env var `DATABASE_URL`. Prisma client is generated on Vercel via `packages/db` `postinstall: prisma generate`; `schema.prisma` `binaryTargets` includes `rhel-openssl-3.0.x` for the Vercel runtime. Full guide: `docs/DEPLOYMENT.md`.
  ...
  - **Prod DB = teammate's VPS PostgreSQL** *(06-13)* — moved off Neon (now retired). `DATABASE_URL` set manually in Vercel to `postgresql://appuser:<DB_PASSWORD>@187.77.185.220:5433/nxtsft` (the real password lives only in the gitignored `.env`/Vercel env — **never commit it**; the `@` in it must be `%40`-encoded in the URL). Neon integration disconnected. Still accessed via Prisma (ORM unchanged). See Overall Status for the VPS-as-prod follow-ups (firewall/SSL/pooling). ⚠️ SECURITY: the literal password was committed here in a prior session (`dbfb75d`, already on `origin/main`) and must be rotated — see the pre-push audit note.
...
### Designer/Decor monthly listing billing *(07-07)*
Sub-project 2 of 3 (spec: `docs/superpowers/specs/2026-07-07-designer-decor-billing-design.md`; plan: `docs/superpowers/plans/2026-07-07-designer-decor-billing.md`). Satisfies the PDF's Home Interiors "Monthly Listing Plan (₹400–1,000/month)".
- [x] `planTypeSchema` gains `designer`/`decor` (`packages/trpc/src/sanitize.ts`); two DB-backed `Plan` rows seeded (`designer-monthly`/`decor-monthly`, ₹499/mo — `packages/db/prisma/seed.ts`), so admins can re-price via the Plans Manager instead of a code change.
- [x] New `subscriptions.{createBusinessOrder, createBusinessPayUOrder, verifyBusinessPayment, myBusinessSubscription}` (`subscriptions.ts`) — mirror the existing owner-plan procedures but source the plan from the DB, not the hardcoded array. Razorpay HMAC verify + PayU flow reuse the existing, proven paths (PayU callback route needed **zero** changes — its generic `owner_subscription` branch already handles it). `myBusinessSubscription` is type-scoped (unlike the pre-existing type-blind `myCurrent`).
- [x] Frontend: "Listing Plan" section in `MyBusinessTab.tsx` (Razorpay/PayU checkout, reusing the `CreditsTab` pattern; decor-only owners correctly see the decor plan). Admin `PlansManager.tsx` now surfaces Designer/Decor plan groups (fix from the final review — the spec had wrongly assumed it already did).
- [x] **Track-only, no enforcement** — deliberately matches how the app's existing owner-rent/owner-sell plans already behave (they enforce nothing either).
- [x] Built via subagent-driven development (5 tasks + 1 review-driven fix), each task implement → review → whole-branch review, all clean. Verified live (tsc/lint/build + real Razorpay order round-trip + direct-DB `Subscription` UI check); all throwaway rows cleaned up. **Note:** this dev env has *live* Razorpay keys — verification created one unpaid test order in the real dashboard (user OK'd this as harmless).

### PG owner media package requests *(07-07)*
Sub-project 3 of 3 (spec: `docs/superpowers/specs/2026-07-07-pg-media-package-requests-design.md`; plan: `docs/superpowers/plans/2026-07-07-pg-media-package-requests.md`). Satisfies the PDF's "For PG Owners" Starter (₹2,000–4,500) / Premium (₹5,000–8,000) packages.
- [x] **Lead-capture only, by design** (user's explicit v1 call): the owner requests a package; staff follow up offline to shoot/schedule/quote/collect payment. No online checkout — the deliverable is a real-world creative-production service.
- [x] New `properties.requestMediaPackage` (`properties.ts`) — validates ownership (`FORBIDDEN`) and PG-type (`BAD_REQUEST`), then creates an `Enquiry` row (`source: "PG Media Package"`, includes the owner's `city`) + a confirmation notification. Mirrors the existing `reportIssue` pattern; reuses the `Enquiry` model so it lands in the existing admin queue with no new admin screen.
- [x] Frontend: "Get Professional Media" button + modal on PG-type cards in `MyListingsTab.tsx`; admin `EnquiriesTab.tsx` gains a "Source" column/badge so these are distinguishable.
- [x] Built via subagent-driven development (3 tasks + 1 review-driven follow-up), whole-branch review = "Ready to merge: Yes". Verified live (incl. the cross-owner `FORBIDDEN` path and a real Playwright browser session for the admin column); all throwaway rows cleaned up.

### Pre-push audit *(07-07)*
- [x] DB-safe to push: schema diff vs `origin/main` is only the 2 nullable `userId` columns + indexes/FKs + 2 `User` reverse-relations + 1 comment; all already present in the live prod DB (verified). Deploy runs `prisma generate` only (never `db push`/`migrate`), so the push cannot alter the DB. `prisma validate` ✓, full build ✓ (56 routes, 0 errors). Reviewer guide: `docs/REVIEW_GUIDE.md`.
- [x] Known PDF gaps **not** in this batch (out of scope of the 4 features): "Distance" on the PG card, "Nearby Colleges/Companies" on PG detail (`Location.nearbyPlaces` exists, unrendered), designer lead *analytics* charts (lead list is built), per-lead "property location" (approximated with buyer city). PDF's own "future/optional" items excluded.

### Removed admin approval for Home Sellers *(08-20)*
- [x] **Home Sellers are now active immediately on signup — no admin-approval queue.** Applies to all three paths that create a Home Seller: `registerSeller` (direct `/register`), `completePhone` seller branch (Google "I'm selling"), and `applyAsSeller` (existing buyer → seller on `/list`). Shared helper renamed `convertToPendingSeller` → `convertToSeller` (`verified: true`, keeps the session live instead of signing out).
- [x] **Deliberately additive — nothing removed.** Agents / Partners still go through the approval queue (RERA verification) unchanged; the Seller Approvals tab, `admin.users.verify`, and the RERA-gated *listing* approval (`admin.properties.approve` → Pending until approved) all stay exactly as before. `registerSeller` now returns `{ pendingApproval, token?, user? }`; agent path returns `pendingApproval: true` (queued), seller returns `false` + a session.
- [x] Frontend: `/register` seller flow signs in + hard-navigates to `/list` (window.location, avoiding the Router-Cache redirect bug), toggle labels shortened to **Buyer** / **Seller** (per boss); `/list` "switch to seller" converts in place and drops straight into the wizard (removed the "Application submitted" screen); `/login` Google-seller + `SellerApprovalsTab` copy updated. Agent copy/flow untouched.
- [x] One-time backfill (`packages/db/prisma/backfill-approve-home-sellers.ts`, scoped to `role: "home-seller"` only) auto-approves accounts already stuck in the old queue.
- [x] Verified: `tsc --noEmit` (trpc + web) clean, `next lint` clean, `pnpm --filter @nxtsft/web build` clean. Live end-to-end verification against prod pending.

### Fix: long-title listings 400 on their own detail page *(07-15)*
- [x] `generateSlug` had **no length cap**, so a long title → ~110-char slug, but `properties.get` capped its input at `safeString(100)` → `properties.get` 400s ("Inputs exceeds 100 chars") and the detail page can't load. (`properties.similar` already allowed 300 — inconsistent.)
- [x] Fix (`packages/trpc/src/routers/properties.ts`): raised `get` input to `safeString(300)` so existing long-slug listings load; capped `generateSlug` (collapse repeated dashes, slice base to 80, strip trailing dash) so new slugs stay ≤ ~94 chars. Verified: `tsc` (trpc) + `pnpm --filter @nxtsft/web build` clean.

### Fix: listing price showed ₹NaN / saved wrong amount *(07-15)*
- [x] The `/list` price field was `type="text"` with a "75 Lakh / 1.2 Cr" placeholder, so it stored raw text. Preview did `Number("75 Lakh")` → **NaN** ("₹NaN"), and submit did `parseInt("75,00,000")` → **75** (comma stops it) — the saved price was garbage. (Area/built-up are `type="number"`, so they were fine; no PG-deposit field here.)
- [x] Fix (`apps/web/src/app/list/page.tsx`): price input now strips to digits on input (`replace(/\D/g,"")`), displays Indian comma-grouped (`75,00,000`), `inputMode="numeric"`, placeholder `e.g. 75,00,000`, plus a `priceInWords()` hint (`≈ ₹75 Lakh`). Preview + submit + success screen all format via `Number(...).toLocaleString("en-IN")` on the now-clean digits. Verified: `tsc` + `pnpm --filter @nxtsft/web build` clean.

### Fix: "List a Property" bounces logged-in sellers to /login *(07-15)*
- [x] **Root cause (confirmed by live repro): Next.js Router Cache replaying a stale middleware redirect.** A logged-OUT visit to `/list` gets bounced by middleware to `/login`, and Next caches that `/list → /login` redirect in the client Router Cache. After signing in, a **soft** (client) navigation to `/list` — the sidebar/header "List a Property" `<Link>`, and the post-login `router.push`/`replace` — replays the cached redirect → back to `/login`, in a loop. **Typing the URL (hard nav) works** because it bypasses the cache; that hard-vs-soft difference is what pinned it. Nothing to do with cookies/roles/device (all verified fine: valid-role cookie → `/list` = 200 via curl + headless).
- [x] Separately surfaced the **`ananya@example.com` demo account is on the invalid legacy role `customer`** (only 1 such account in prod; all 1,385 real home-sellers are valid). That role additionally crashes the login page at `ROLE_META["customer"].portal`. Data fix (role → `home-seller`) still pending — blocked from the prod write; can do with go-ahead or via SA portal.
- [x] **Fix:** `apps/web/src/app/login/page.tsx` now navigates post-login with `window.location` (hard nav) instead of the Next router, so the stale cached redirect is dropped. The auto-redirect effect also hard-navigates, so an in-session bounce to `/login` self-heals to the intended page instead of looping. Added `safeInternalPath()` to keep the `?redirect` target internal (no open redirect) now that we hard-navigate. Verified: `tsc --noEmit` + `pnpm --filter @nxtsft/web build` clean.

### WhatsApp — admin alert on new user registration *(07-15)*
- [x] **Root cause of "not receiving auto WhatsApp on new registration": the feature was never built.** `notify()` writes only an in-app 🔔 notification; the sole WhatsApp on signup was the OTP to the *registering user*. Admins got an in-app bell for seller/agent signups only (not buyers), never a WhatsApp.
- [x] New `notifyAdminNewUser()` in `bhashsms.ts` — sends the owner an alert (params `[name, phone, city, role]`) via `sendTemplateIfConfigured`, env-gated on `BHASHSMS_TEMPLATE_ADMIN_NEW_USER` (approved template name) **and** `ADMIN_ALERT_WHATSAPP` (owner's number). Silent no-op until both are set, so it can ship now.
- [x] Wired (awaited, best-effort) into **every completed-signup path** in `auth.ts`: `register` (buyer), `registerSeller` (seller/agent), and `completePhone` (both Google branches — buyer & seller). Deliberately NOT fired at the phone-less `googleSignIn` stub — Google users get one accurate alert (with phone + final role) once they finish `completePhone`, mirroring where `createSignupLead` already fires.
- [x] Documented in `docs/whatsapp-bhashsms-templates.md` (table row + wording + wiring + the "needs BOTH env vars" caveat) and `.env.example`.
- [x] Verified: `tsc --noEmit` (trpc) + `pnpm --filter @nxtsft/web build` clean. **Delivery not exercised end-to-end** — it's an intentional no-op until the client creates + approves the `admin_new_user` BhashSMS template and sets `BHASHSMS_TEMPLATE_ADMIN_NEW_USER` + `ADMIN_ALERT_WHATSAPP` in Vercel (BhashSMS creds are already live via OTP login).
- [x] **Sender-ID split (delivery blocker fix):** `sendWhatsAppTemplate` sent BOTH auth (OTP) and utility templates from one sender (`BHASHSMS_SENDER` → `BhashSoftwareLab`). Made sender `stype`-aware so a distinct utility sender is possible (`BHASHSMS_SENDER_UTILITY`), overridable per account.
- [x] **Final delivery fix (Vercel logs): HTTPS `fetch failed` → use plain http + timeout.** After the endpoint/sender fix, the app still logged `[bhashsms] ... not delivered to 7989648797: fetch failed` with `auth.register` hanging 17s. Cause: the util endpoint was set to `https://`, but bhashsms.com's HTTPS chain is missing an intermediate cert — browsers tolerate it (so the direct https test worked) but Node/undici on Vercel rejects it. The live OTP proves plain `http` works from Vercel. Switched `UTIL_ENDPOINT` to `http://` and added an 8s `AbortSignal.timeout` to every send so a slow/failed gateway can't hang the triggering request again. This was the actual blocker for real signups (direct API test + template + env were all already correct).
- [x] **Live diagnosis via direct BhashSMS API tests (client-run) — the real root cause was the wrong endpoint+sender.** BhashSMS uses a **different endpoint AND sender per template category**: auth/OTP → `sendmsg.php` + `BhashSoftwareLab`; **utility → `sendmsgutil.php` + `BUZWAP`**. The code only ever used `sendmsg.php`, so every utility send failed (`sendmsg.php` returns `Marketing Templates Not Allowed` for these). A `sendmsgutil.php` + `BUZWAP` test returned `S.937806` (success). **Fixed `sendWhatsAppTemplate`** to pick endpoint+sender from `stype`: auth → `sendmsg.php`/`BhashSoftwareLab`, utility → `sendmsgutil.php`/`BUZWAP` (both env-overridable). Also unblocks all the other never-delivered utility templates (seller_welcome, new_lead_alert, etc.). Docs corrected in `.env.example` + templates doc. **Still on client:** confirm the `admin_new_user` template is accepted on the util endpoint (test URL) and set `BHASHSMS_TEMPLATE_ADMIN_NEW_USER` to its exact name, then redeploy.

### Admin — listing card "View" button *(07-15)*
- [x] Each admin Listings ("All Submissions") card now has an explicit **View** action (Eye icon → opens the public `/properties/{slug}` page in a new tab), shown whenever the listing has a public page (`it.slug`). The card title was already a link to the same page; the button just makes it discoverable. `ListingsTab.tsx` only. Verified: `tsc --noEmit` + `pnpm --filter @nxtsft/web build` clean.
- Watermark double-layer checked while here: **no double** in current code — teammate already landed the fix (commits `6597109`→`13beb0d`). Uploads bake the mark; `WatermarkOverlay` skips the overlay for R2-hosted (`.r2.dev` / `NEXT_PUBLIC_R2_HOST`) images, so baked photos get a single mark and non-R2 seed/placeholder images get the single overlay. Caveat: if R2 serving ever moves to a custom domain, `R2_PUBLIC_URL`/`NEXT_PUBLIC_R2_HOST` must be set to it at build time or the overlay re-doubles onto baked photos.

### Admin — delete a user (tester account reset) *(07-15)*
- [x] New `admin.users.deleteAccount` (super-admin only, `packages/trpc/src/routers/admin.ts`) — a hard delete built for testers to fully remove a buyer/seller/agent (or staff) so the same person can register again. Frees the `@unique` `email`/`phone`/`slug` that a deactivate never did. Guards: can't delete yourself, can't delete another `super-admin`, and the caller must re-type the target's email (server-side re-check).
- [x] **Why a transaction, not `user.delete()`:** most child rows FK-block a user with the default `Restrict` rule (Property owner, Lead, Payment, Commission, SalesActivity, Review, Message, Ticket, Campaign, WaBroadcast, CmsPage, plus the user's Listings), and `Session`/`SiteVisit`/`CreditTransaction` carry a `userId` with no FK at all — so the mutation clears those explicitly in dependency order (escalations → listings/reviews/site-visits → leads/messages/payments/tickets/commissions/etc. → properties → user), then lets the DB cascade/SetNull the rest (favorites, subscriptions, notifications, KYC docs, referrals, push subs, edit requests, supervisees, agent-marketed listings, views, directory ownership, enquiry/escalation assignments). Writes an `AuditLog` (`action: "user_deleted"`).
- [x] Frontend: "Delete a user — danger zone" panel in the **Dev Tools tab** (`DevTab.tsx`), **super-admin-only** (`session.role`). Search by name/email/phone via `admin.users.list` → red Delete → modal requiring the exact email typed to arm "Delete permanently".
- [x] Verified: `tsc --noEmit` clean across `trpc`/`web`, `next lint` clean, `pnpm --filter @nxtsft/web build` clean (all routes incl. admin-portal). **Not** live-exercised: driving a real delete would destroy rows on the shared VPS/prod DB — the deletion order was instead derived exhaustively from `schema.prisma`. A controlled throwaway-account smoke test is available on request.
- Known limitation (acceptable for test-consumer resets): bare staff-attribution columns with no FK (`Lead.assignedToId`/`supervisorId`, `*.reviewedById`, `Ticket.assignedTo`, `AssignmentHistory`, `Escalation.raisedById`) are left as-is; and other buyers' leads on a deleted seller's listing survive with a null property link (schema `SetNull`).
