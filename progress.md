...
- [x] **Sender-ID split (delivery blocker fix):** `sendWhatsAppTemplate` sent BOTH auth (OTP) and utility templates from one sender (`BHASHSMS_SENDER` → `BhashSoftwareLab`), but BhashSMS utility templates need a different sender (`BUZWAP`). A single env can't serve both, so a utility send (like `admin_new_user`) would go out under the OTP sender and be rejected. Now sender is `stype`-aware: auth keeps `BHASHSMS_SENDER || "BhashSoftwareLab"` (OTP untouched); utility uses `BHASHSMS_SENDER_UTILITY || "BUZWAP"`, overridable per BhashSMS's actual utility sender. This also unblocks the other wired-but-never-delivered utility templates (seller_welcome, new_lead_alert, etc.). Documented in `.env.example` + templates doc.
