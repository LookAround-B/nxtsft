...
/**
 * Report an interior-designer listing. Mirrors PropertyReport — flag reasons
 * submit anonymously; "Request More Info" collects optional contact. All go to
 * staff as Enquiry rows via interiorDesigners.reportIssue.
 */
export function DesignerReport({ designerId, className }: { designerId: string; className?: string }) {
  const [done, setDone] = useState(false);
  const [showForm, setShowForm] = useState(false);
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");

  const report = trpc.interiorDesigners.reportIssue.useMutation({
    onSuccess: () => {
      setDone(true);
      setShowForm(false);
  ...
  function onClick(reason: Reason) {
    ...
    report.mutate({ id: designerId, reason });
  }
  ...
  function submitMoreInfo() {
    ...
    report.mutate({ id: designerId, reason: "more_info", name: name.trim() || undefined, phone: phone.trim() || undefined });
  }
