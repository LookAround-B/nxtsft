      ...
      <RptSection
        ...
        onExport={() =>
          dlCSV("subscriptions.csv",
            ["ID", "User", "Plan", "Amount (₹)", "Category", "City", "State", "Builder", "Supervisor", "Sales", "Type", "Status", "Subscribed On", "Due Date", "Latest Comment"],
            fSubs.map((s) => [s.id, s.userName, s.plan, s.amount, s.category, s.city, s.state, s.builder, s.supervisor, s.salesStaff, s.type, s.status, s.subscribedOn, s.dueDate, s.latestComment]),
          )
        }
      >
        {fSubs.length === 0 ? (
          ...
          <div className="overflow-x-auto"><table className="portal-table">
            <thead>
              <tr>
                ...
                <th>Sales</th><th>Subscribed</th><th>Due</th><th>Latest Comment</th>
              </tr>
            </thead>
            <tbody>
              {fSubs.map((s, i) => (
      ...
      <RptSection
        ...
        onExport={() =>
          dlCSV("site-visits.csv",
            ["ID", "Lead Name", "Property", "City", "State", "Builder", "Supervisor", "Sales Staff", "Category", "Scheduled On", "Status", "Latest Comment"],
            fVisits.map((v) => [v.id, v.leadName, v.property, v.city, v.state, v.builder, v.supervisor, v.salesStaff, v.category, v.scheduledOn, v.status, v.latestComment]),
          )
        }
      >
        {fVisits.length === 0 ? (
          ...
          <div className="overflow-x-auto"><table className="portal-table">
            <thead>
              <tr>
                ...
                <th>Scheduled</th><th>Status</th><th>Latest Comment</th>
              </tr>
            </thead>
            <tbody>
              {fVisits.map((v, i) => (
