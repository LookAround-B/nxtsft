...
type Reason = "broker" | "rented" | "sold" | "wrong_info" | "more_info";

const REASONS: { reason: Reason; label: string }[] = [
  { reason: "broker", label: "Listed by Broker" },
  { reason: "rented", label: "Rented Out" },
  { reason: "sold", label: "Sold Out" },
  { reason: "wrong_info", label: "Wrong Info" },
  { reason: "more_info", label: "Request More Info" },
];

/**
 * "Report what was not correct in this property" (GOL-123).
 * Flag reasons submit anonymously; "Request More Info" collects optional contact.
 * All go to staff as Enquiry rows via properties.reportIssue.
 */
export function PropertyReport({ propertyId, className }: { propertyId: string; className?: string }) {
  const [done, setDone] = useState(false);
  const [showForm, setShowForm] = useState(false);
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");

  const report = trpc.properties.reportIssue.useMutation({
    onSuccess: () => {
      setDone(true);
      setShowForm(false);
  ...
  function onClick(reason: Reason) {
    ...
    report.mutate({ propertyId, reason });
  }
  ...
  function submitMoreInfo() {
    ...
    report.mutate({ propertyId, reason: "more_info", name: name.trim() || undefined, phone: phone.trim() || undefined });
  }
