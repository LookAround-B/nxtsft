...
export function PerformanceTab() {
  const perfQ = trpc.supervisor.performance.useQuery();
  const monthlyTarget = perfQ.data?.monthlyTarget ?? 8;
  const reps = perfQ.data?.reps ?? [];
  ...
  return (
    <>
      ...
      {perfQ.isLoading ? (
        ...
        <Section title="Leaderboard — Closed MTD">
          <p className="py-8 text-center text-sm text-muted-foreground">No sales reps found.</p>
        </Section>
      ) : (
        <>
          {/* Leaderboard */}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hidden sm:block">
                      ...
                      <Sparkline values={m.weeklyTrend} />
                    </div>
                    {/* Closed & gap */}
