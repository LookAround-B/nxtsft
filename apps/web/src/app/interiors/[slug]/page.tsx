...
import {
  Sofa, MapPin, Globe, CheckCircle2, Calendar, ArrowLeft, IndianRupee,
  Phone, Mail, MessageCircle, Coins, ShieldCheck, ChevronRight, Clock, Palette,
...
type Designer = {
  ...
  startingBudget: number | null; designStyles: string[]; servicesOffered: string[];
  areasServed: string[]; portfolioImages: string[]; portfolioVideos: string[];
  workingHours: string | null; website: string | null;
  virtualTourUrl: string | null; walkthroughVideoUrl: string | null;
};

function ContactCard({ designer, session, credits, refreshCredits }: {
  designer: Designer;
  ...
  const unlock = trpc.interiorDesigners.unlockContact.useMutation({
    onSuccess: (data) => {
      setContact({ phone: data.phone, email: data.email });
      toast.success("Designer contact unlocked! 1 credit used.");
      void refreshCredits();
    },
  ...
  const handleUnlock = () => {
    ...
    if (credits <= 0) {
      router.push("/pricing");
      toast.info("You need credits to unlock designer contacts.");
      return;
    }
    unlock.mutate({ id: designer.id });
  };

  const initials = designer.companyName.split(" ").map((s) => s[0] ?? "").join("").slice(0, 2).toUpperCase();

  return (
    <div className="rounded-2xl border border-border bg-white p-6 shadow-sm">
      <h3 className="font-display text-base font-bold text-navy">Contact Designer</h3>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <div>
          <div className="font-semibold text-foreground">{designer.companyName}</div>
          {designer.verified && (
        ...
        <div className="mt-4 flex items-center gap-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-2.5 text-xs text-amber-800">
          ...
          <span>
            {session
              ...
              : "Sign in to unlock designer contact."}
          </span>
        </div>
      )}
      ...
      {contact ? (
        ...
        >
          {unlock.isPending ? (
            ...
            <>
              <Phone size={16} />
              {session ? "Unlock Designer Contact" : "Sign in to Contact"}
              {session && credits > 0 && (
...
function SimilarDesigners({ slug }: { slug: string }) {
  const { data } = trpc.interiorDesigners.similar.useQuery({ slug }) as { data: Designer[] | undefined };
  if (!data || data.length === 0) return null;
  return (
    <div className="rounded-2xl border border-border bg-white p-6">
      <h2 className="mb-4 font-display text-lg font-bold text-navy">Similar Interior Designers</h2>
...
export default function InteriorDesignerPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = use(params);
  const { session, credits, refreshCredits } = useAuth();
  const { data: designer, isLoading } = trpc.interiorDesigners.publicGet.useQuery({ slug }) as {
    data: Designer | null | undefined;
    isLoading: boolean;
  };

  const [saved, setSaved] = useState(false);
  const addFavorite = trpc.users.addDesignerFavorite.useMutation({
    onSuccess: () => { setSaved(true); toast.success("Saved to your favourites!"); },
    onError: (e) => toast.error(e.message),
  });
  const removeFavorite = trpc.users.removeDesignerFavorite.useMutation({
  ...
  function toggleSave() {
    if (!designer) return;
    if (!session) { toast.info("Sign in to save designers."); return; }
    if (saved) removeFavorite.mutate({ designerId: designer.id });
    else addFavorite.mutate({ designerId: designer.id });
  }

  async function share() {
    const url = typeof window !== "undefined" ? window.location.href : "";
    try {
      if (navigator.share) await navigator.share({ title: designer?.companyName, url });
      else { await navigator.clipboard.writeText(url); toast.success("Link copied to clipboard."); }
    } catch { /* user dismissed share sheet */ }
  }

  const recordView = trpc.interiorDesigners.recordView.useMutation();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      recordViewRef.current({ designerId, durationSec });
    };
  }, [designerId]);
  ...
  if (!designer) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="font-semibold text-navy">Designer not found</p>
        <Link href="/interiors" className="text-sm text-accent hover:underline">← Back to directory</Link>
      </div>
    );
  }

  const images = designer.portfolioImages.length > 0 ? designer.portfolioImages : [];
    ...
    <main className="min-h-screen bg-[oklch(0.97_0.01_260)]">
      ...
      <div className="border-b border-border bg-white">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
          <Link href="/interiors" className="mb-4 flex items-center gap-1 text-xs text-muted-foreground hover:text-accent">
            <ArrowLeft size={12} /> Back to Home Interiors
          </Link>
          <div className="flex flex-wrap items-start gap-5">
            <div className="grid h-16 w-16 flex-shrink-0 place-items-center rounded-2xl border border-border bg-accent/10 text-accent">
              {designer.logo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={designer.logo} alt={designer.companyName} className="h-14 w-14 rounded-xl object-contain" />
              ) : (
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="font-display text-2xl font-black text-navy">{designer.companyName}</h1>
                {designer.verified && (
                  <span className="flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700">
                    <CheckCircle2 size={11} /> Verified Designer
                  </span>
                )}
              </div>
              <div className="mt-1 flex flex-wrap gap-3 text-xs text-muted-foreground">
                {(designer.city || designer.state) && (
                  <span className="flex items-center gap-1"><MapPin size={11} />{[designer.city, designer.state].filter(Boolean).join(", ")}</span>
                )}
                {designer.yearsExperience != null && (
                  <span className="flex items-center gap-1"><Calendar size={11} />{designer.yearsExperience}+ years experience</span>
                )}
                {designer.workingHours && (
                  <span className="flex items-center gap-1"><Clock size={11} />{designer.workingHours}</span>
                )}
                {designer.website && (
                  <a href={designer.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-accent">
                    <Globe size={11} />{designer.website.replace(/^https?:\/\//, "")}
                  </a>
                )}
              </div>
              {designer.description && (
                <p className="mt-2 max-w-2xl text-sm text-muted-foreground">{designer.description}</p>
              )}
            </div>
            <div className="flex flex-col items-end gap-3">
              ...
              <div className="flex gap-4">
                {[
                  { label: "Projects Done", value: designer.projectsCompleted },
                  { label: "Design Styles", value: designer.designStyles.length },
                  { label: "Areas Served",  value: designer.areasServed.length },
                ].map(({ label, value }) => (
            ...
            <div className="rounded-2xl border border-border bg-white p-6">
              ...
              {images.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {images.map((src, i) => (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img key={i} src={src} alt={`${designer.companyName} project ${i + 1}`} className="aspect-square w-full rounded-xl object-cover" />
                  ))}
                </div>
              )}
            </div>

            {/* 360° tour & walkthrough videos */}
            {(designer.virtualTourUrl || designer.walkthroughVideoUrl || designer.portfolioVideos.length > 0) && (
              <div className="rounded-2xl border border-border bg-white p-6">
                <h2 className="mb-4 font-display text-lg font-bold text-navy">Tour &amp; Videos</h2>
                <div className="space-y-4">
                  {designer.virtualTourUrl && (
                    <div>
                      ...
                      <div className="aspect-video overflow-hidden rounded-xl border border-border">
                        <iframe src={designer.virtualTourUrl} className="h-full w-full" allowFullScreen title="360° virtual tour" />
                      </div>
                    </div>
                  )}
                  {designer.walkthroughVideoUrl && (
                    <div>
                      <div className="mb-1.5 flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                        <Play size={12} /> Walkthrough Video
                      </div>
                      <div className="aspect-video overflow-hidden rounded-xl border border-border">
                        <iframe src={designer.walkthroughVideoUrl} className="h-full w-full" allowFullScreen title="Walkthrough video" />
                      </div>
                    </div>
                  )}
                  {designer.portfolioVideos.length > 0 && (
                    <div>
                      <div className="mb-1.5 flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                        <Video size={12} /> Project Videos
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {designer.portfolioVideos.map((src, i) => (
