...
export const metadata: Metadata = {
  ...
  description:
    "Learn how to identify real estate fraud and scams. NxtSft.com's guide to staying safe when buying, renting, or selling property online.",
};
...
const PROTECTIONS = [
  {
    ...
    desc: 'Our team manually verifies select listings for title authenticity, RERA registration, and ownership documents. Look for the "Verified" badge.',
  },
  {
    icon: CheckCircle2,
    title: "Agent credentialing",
    desc: "Agents on NxtSft.com must submit RERA agent registration details and valid ID proof before accessing lead tools.",
  },
  {
    icon: Eye,
    title: "Fraud detection algorithms",
    desc: "Our systems flag suspicious activity patterns — bulk listings, duplicate property photos, abnormal pricing, and unusual contact behaviour.",
  },
  {
    icon: Lock,
    title: "Masked contact numbers",
    desc: "Phone numbers are masked in listings until a Credit is used. This discourages mass data harvesting by bad actors.",
  },
  {
    icon: MessageSquare,
    title: "In-Platform messaging",
    desc: "All communication on NxtSft.com is through our Platform, creating a record that can be reviewed in case of a dispute.",
  },
  {
    icon: AlertTriangle,
    title: "User report mechanism",
    desc: 'Any user can flag a suspicious listing or user via the "Report" button. Our trust team reviews all reports within 24 hours.',
  },
];
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-5xl px-6 py-16">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-xs font-bold uppercase tracking-widest text-amber-600">
              Safety & Fraud Advisory
            </div>
          </div>
          <h1 className="mt-3 font-display text-4xl font-black text-navy sm:text-5xl">
            Protect Yourself from
            <br className="hidden sm:block" /> Real Estate Fraud
          </h1>
          <p className="mt-5 max-w-2xl text-base text-muted-foreground">
            Real estate transactions involve large sums of money, making them a prime target for
            fraudsters. This advisory helps you recognise, avoid, and report fraud when buying,
            renting, or selling property on NxtSft.com.
          </p>
          ...
          <div className="mt-8 inline-flex flex-col gap-3 rounded-2xl border-2 border-red-200 bg-red-50 px-6 py-5 sm:flex-row sm:items-center sm:gap-6">
            <div>
              ...
              <div className="mt-0.5 text-xs text-red-600">
                Report immediately — do not make any payments.
              </div>
            </div>
            <a
      ...
      <div className="border-b border-amber-200 bg-amber-50">
        <div className="mx-auto max-w-5xl px-6 py-4">
          <p className="text-xs text-amber-800">
            <strong>Disclaimer:</strong> NxtSft.com is an online marketplace and intermediary. We do
            not own, sell, or lease any property listed on the Platform and are not party to any
            transaction. While we work hard to keep the Platform safe, we cannot guarantee the
            authenticity of every listing or the conduct of every user. Always exercise independent
            due diligence before making any payment or signing any agreement.
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl px-6 py-14 space-y-16">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2">
            {FRAUD_TYPES.map((f) => (
              <div key={f.tag} className={`rounded-2xl border p-6 ${f.color}`}>
                <span
                  className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${f.tagColor}`}
                >
                  {f.tag}
                </span>
                  ...
                  <ol className="space-y-1">
                    {f.howItWorks.map((step, i) => (
                      <li key={i} className="flex gap-2 text-xs leading-relaxed">
                        <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-current/20 text-[9px] font-bold">
                          {i + 1}
                        </span>
                        {step}
                      </li>
        ...
        <section>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {RED_FLAGS.map((f) => (
              <div
                key={f.title}
                className="flex gap-4 rounded-2xl border border-border bg-white p-5 transition hover:border-red-200 hover:shadow-sm"
              >
        ...
        <section>
          ...
          <h2 className="font-display text-2xl font-black text-navy sm:text-3xl">
            How NxtSft.com works to keep you safe
          </h2>
        ...
        <section className="rounded-2xl border-2 border-navy/10 bg-navy/[0.03] p-8">
          <div className="mb-2 text-xs font-bold uppercase tracking-widest text-navy">
            Action plan
          </div>
          <h2 className="font-display text-2xl font-black text-navy">
            If you suspect or have been a victim of fraud
          </h2>
          ...
          <div className="mt-6 space-y-4">
            {[
              {
                step: "01",
                title: "Stop all communication and payments",
                desc: "Do not send any further money. Block the suspect's contact on all channels and preserve all communication records (screenshots, emails, messages).",
              },
              {
                step: "02",
              ...
              },
              {
                ...
                desc: "Visit your nearest police station or file a complaint online at the National Cyber Crime Reporting Portal (cybercrime.gov.in). Provide all evidence including payment receipts and communication history.",
              },
              {
                step: "04",
                title: "Contact your bank immediately",
                desc: "If you have transferred money, contact your bank's fraud hotline immediately. Report the UPI transaction to NPCI at cybercrime.gov.in or call 1930 (National Cyber Crime Helpline).",
              },
              {
                step: "05",
                title: "File with RERA",
                desc: "For builder or developer fraud, file a complaint with the state RERA authority. RERA provides a formal adjudication mechanism for such disputes.",
              },
            ].map((item) => (
              <div key={item.step} className="flex gap-5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-navy font-display text-sm font-black text-white">
                  {item.step}
                </div>
                <div>
        ...
        <section className="rounded-2xl border border-border bg-white p-8 text-center">
          <h2 className="font-display text-xl font-black text-navy">
            Report a suspicious listing or user
          </h2>
          <p className="mt-3 text-sm text-muted-foreground max-w-lg mx-auto">
            See something that doesn&apos;t look right? Use the "Report" button on any listing, or
            write to us directly. Every report is reviewed by our team.
          </p>
