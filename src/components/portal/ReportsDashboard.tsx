...
import {
  reportUsers, reportSubscriptions, reportSiteVisits, reportAgentRegs, reportTickets,
  REPORT_CATEGORIES, REPORT_CITIES, REPORT_STATES, REPORT_BUILDERS, REPORT_SUPERVISORS, REPORT_SALES,
} from '@/data/reports';
...
export function ReportsDashboard({
  ...
  const matchDims = (item: {
    ...
    builder?: string; supervisor?: string; salesStaff?: string;
  }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          ...
          <Sel label="City"        value={filters.city}       options={REPORT_CITIES}       onChange={(v) => setF('city', v)} />
          <Sel label="State"       value={filters.state}      options={REPORT_STATES}       onChange={(v) => setF('state', v)} />
          <Sel label="Builder"     value={filters.builder}    options={REPORT_BUILDERS}     onChange={(v) => setF('builder', v)} />
          <Sel label="Supervisor"  value={filters.supervisor} options={REPORT_SUPERVISORS}  onChange={(v) => setF('supervisor', v)} locked={!!defaultSupervisor} />
          <Sel label="Sales Staff" value={filters.sales}      options={REPORT_SALES}        onChange={(v) => setF('sales', v)} locked={!!defaultSales} />
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className="rounded-full bg-purple-50 px-3 py-1 text-xs font-bold text-purple-700">{fAgents.length} Agent Regs</span>
        <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-bold text-rose-700">
          {fTickets.length} Tickets · {resolvedTickets.length > 0 ? Math.round((withinTAT / resolvedTickets.length) * 100) : 0}% within TAT
        </span>
      </div>
      ...
      <RptSection
        ...
        count={fUsers.length}
        onExport={() => dlCSV('registered-users.csv',
      ...
      <RptSection
        title="Subscriptions"
        count={fSubs.length}
        onExport={() => dlCSV('subscriptions.csv',
          ...
          <table className="portal-table">
            <thead>
              <tr>
                ...
                <th>Type</th><th>Status</th><th>City</th><th>Supervisor</th><th>Sales</th>
                <th>Subscribed</th><th>Due</th>
              </tr>
            </thead>
            <tbody>
              {fSubs.map((s) => (
                <tr key={s.id}>
                  ...
                  <td>
                    <Badge tone={
                      ...
                      : s.status === 'Follow-up' ? 'warm'
                      : s.status === 'Not Interested' ? 'cold'
                      : 'hot'
                    }>
                      {s.status}
                    </Badge>
      ...
      <RptSection
        ...
        onExport={() => dlCSV('site-visits.csv',
                ...
                <tr key={v.id}>
                  ...
                  <td>
                    <Badge tone={
                      ...
                      : v.status === 'Scheduled' ? 'new'
                      : v.status === 'Cancelled' ? 'warm'
                      : 'hot'
                    }>
                      {v.status}
                    </Badge>
      ...
      <RptSection
        ...
        onExport={() => dlCSV('agent-registrations.csv',
      ...
      >
        {fAgents.length === 0
          ...
          <table className="portal-table">
            <thead>
              <tr>
                <th className="py-2">ID</th><th>Name</th><th>City</th><th>State</th>
                <th>RERA Number</th><th>Registered On</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {fAgents.map((a) => (
      ...
      <RptSection
        ...
        onExport={() => dlCSV('support-tickets.csv',
          ...
          fTickets.map((t) => [
            t.id, t.subject, t.raisedBy, t.category, t.city, t.state,
            t.assignedTo, t.supervisor, t.raisedOn, t.resolvedOn ?? '—',
            t.tatHours, t.actualHours ?? '—', t.status,
