...
export const metadata: Metadata = {
  ...
  description: 'Learn how NxtSft.com uses cookies and similar tracking technologies, what choices you have, and how to manage your preferences.',
};
...
const SECTIONS = [
    ...
    table: [
      {
        ...
        tone: 'blue',
        purpose: 'These cookies are essential for the Platform to function. They enable core features such as user authentication, session management, security, and access to paid features. The Platform cannot operate properly without them.',
        examples: ['Session token (keeps you logged in)', 'CSRF protection token', 'Consent preference cookie (stores your cookie choice)'],
        canDisable: false,
      },
      {
        category: 'Functional',
        tone: 'violet',
        purpose: 'These cookies remember choices you make to provide a personalised experience. They are not essential but significantly improve usability.',
        examples: ['Saved search filters', 'Recently viewed properties', 'Preferred city or region', 'Language and display preferences'],
        canDisable: true,
      },
      {
        category: 'Analytics & Performance',
        tone: 'amber',
        purpose: 'These cookies help us understand how visitors interact with the Platform by collecting and reporting aggregated, anonymised usage data. This helps us improve content, navigation, and features.',
        examples: ['Google Analytics (_ga, _gid, _gat)', 'Hotjar session recordings (hjSession)', 'Internal page-view tracking', 'Feature adoption metrics'],
        canDisable: true,
      },
      {
        category: 'Advertising & Targeting',
        tone: 'rose',
        purpose: 'These cookies are used to deliver advertisements relevant to you and your interests, both on the Platform and on third-party websites. They also limit the number of times you see an ad and help measure ad campaign effectiveness.',
        examples: ['Meta Pixel (_fbp, _fbc)', 'Google Ads conversion tag (gcl_au)', 'Retargeting cookies from ad networks', 'Interest-based profiling cookies'],
        canDisable: true,
      },
    ],
  },
  ...
  },
  {
    ...
    cookieTable: [
      { name: 'nxtsft_cookie_consent', purpose: 'Stores your cookie consent choice (accepted / rejected). Prevents the banner from reappearing.', duration: '12 months', type: 'Strictly Necessary' },
      { name: 'nxtsft_session',        purpose: 'Maintains your authenticated session after login.',                                              duration: 'Session',   type: 'Strictly Necessary' },
      { name: 'nxtsft_csrf',           purpose: 'Cross-site request forgery protection token.',                                                   duration: 'Session',   type: 'Strictly Necessary' },
      { name: 'nxtsft_saved_search',   purpose: 'Persists your most recent search filters for a faster return experience.',                       duration: '30 days',   type: 'Functional' },
      { name: 'nxtsft_recent_props',   purpose: 'Stores IDs of recently viewed property listings.',                                               duration: '7 days',    type: 'Functional' },
      { name: 'nxtsft_city_pref',      purpose: 'Remembers your preferred city for search results.',                                              duration: '90 days',   type: 'Functional' },
      { name: 'nxtsft_ab_variant',     purpose: 'Assigns you to an A/B test group for feature experiments.',                                      duration: '30 days',   type: 'Analytics & Performance' },
    ],
  },
  {
    id: 'manage',
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b border-border bg-white">
        <div className="mx-auto max-w-4xl px-6 py-16">
          ...
          <h1 className="mt-3 font-display text-4xl font-black text-navy sm:text-5xl">Cookie Policy</h1>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
            ...
            <span>Last updated: <strong className="text-foreground">1 June 2026</strong></span>
          </div>
          <div className="mt-6 flex flex-wrap gap-3 text-xs">
            <Link href="/privacy" className="rounded-full border border-border bg-white px-3 py-1.5 font-semibold text-muted-foreground transition hover:border-accent hover:text-accent">
              ← Privacy Policy
            </Link>
            <Link href="/terms" className="rounded-full border border-border bg-white px-3 py-1.5 font-semibold text-muted-foreground transition hover:border-accent hover:text-accent">
              Terms of Use
            </Link>
          </div>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[220px_1fr]">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24 rounded-2xl border border-border bg-white p-5">
              <div className="mb-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Contents</div>
          ...
          <article className="space-y-10">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-4 text-sm text-amber-800">
              <strong>Quick summary:</strong> We use strictly necessary cookies at all times. Functional, analytics, and advertising cookies are only placed with your consent. You can reject non-essential cookies at any time via the banner or your browser settings.
            </div>
            ...
            {SECTIONS.map((s) => (
              <section key={s.id} id={s.id} className="scroll-mt-24">
                ...
                {'table' in s && s.table && (
                  <div className="mt-4 space-y-4">
                    {s.table.map((row) => {
                      ...
                      return (
                        <div key={row.category} className={`rounded-xl border p-5 ${t.border} ${t.bg}`}>
                          <div className="mb-2 flex flex-wrap items-center gap-2">
                            <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ${t.badge}`}>
                              {row.category}
                            </span>
                            {!row.canDisable && (
                          ...
                          <p className="mb-3 text-sm leading-relaxed text-foreground/75">{row.purpose}</p>
                ...
                {'bodyAfterSubsections' in s && s.bodyAfterSubsections && s.bodyAfterSubsections.map((p: string, i: number) => (
                  <p key={i} className="mt-3 text-sm leading-relaxed text-foreground/75">{p}</p>
                ))}
                    ...
                    <table className="w-full border-collapse text-sm">
                      ...
                      <tbody>
                        {s.cookieTable.map((row, i) => (
                          <tr key={row.name} className={`border-b border-border text-xs ${i % 2 === 0 ? 'bg-white' : 'bg-secondary/20'}`}>
                            <td className="px-3 py-3 font-mono font-semibold text-navy">{row.name}</td>
                ...
                {'bodyAfterList' in s && s.bodyAfterList && (s.bodyAfterList as string[]).map((p, i) => (
                  <p key={i} className="mt-3 text-sm leading-relaxed text-foreground/75">{p}</p>
                ))}
