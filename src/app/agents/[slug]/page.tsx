...
import {
  Star, Phone, MessageCircle, Lock, ShieldCheck,
  MapPin, Globe, Award, ArrowLeft, Clock, TrendingUp,
        ...
        <div className="mx-auto max-w-5xl px-5 py-10 sm:px-6 sm:py-14">
          ...
          <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-start">
            ...
            <div className="relative shrink-0">
              <div className={`flex h-24 w-24 items-center justify-center rounded-3xl font-display text-3xl font-black text-white sm:h-28 sm:w-28 ${agent.color}`}>
                {agent.initials}
              </div>
              {agent.verified && (
            ...
            <div className="flex-1">
              ...
              <div className="mt-1 text-sm text-muted-foreground">{agent.role} · Partner since {agent.since}</div>
          ...
          <div className="lg:col-span-2">
            ...
            {agentListings.length > 0 ? (
                ...
                {agentListings.map((p) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="mt-0.5 font-display text-base font-bold leading-tight text-navy">{p.title}</h3>
                      <div className="mt-2 flex items-end justify-between">
                        <div className="font-display text-lg font-black text-accent">{p.priceLabel}</div>
                        <div className="text-[11px] text-muted-foreground">{p.bhk} · {p.area} sqft</div>
                      </div>
                    </div>
                  </Link>
                ))}
              ...
              <div className="mt-5 rounded-2xl border border-dashed border-border bg-white p-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {agent.name.split(' ')[0]} has closed {agent.deals} deals across {agent.cities.slice(0, 2).join(' and ')}.
                </p>
              </div>
            )}
            ...
            {totalValue > 0 && (
              <div className="mt-5 rounded-xl border border-border bg-secondary/30 px-4 py-3 text-sm">
                ...
                <span className="font-bold text-navy">₹{(totalValue / 10000000).toFixed(2)} Cr</span>
              </div>
            )}
          </div>
