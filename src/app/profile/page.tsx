...
import {
  Home, Heart, Calendar, Calculator, MessageCircle, Bell, Phone,
  Mail, MapPin, Shield, LogOut, Edit2, ChevronRight,
  CheckCircle2, Clock, Star, Sparkles, Building2, CreditCard,
  BarChart2, Settings2, Users, Smartphone, Target, FolderOpen,
  RefreshCcw, Satellite, type LucideIcon,
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const togglePref = (key: keyof typeof prefs) => {
    setPrefs((p) => {
      ...
      toast.success(`${key.charAt(0).toUpperCase() + key.slice(1)} notifications ${next[key] ? 'enabled' : 'disabled'}`);
      return next;
    });
  };
    ...
    <div className="min-h-screen bg-[oklch(0.97_0.01_260)] pb-24 md:pb-0">
      ...
      <div className="relative h-40 overflow-hidden bg-gradient-to-r from-navy via-navy-deep to-accent sm:h-56">
        <div className="absolute inset-0" style={{
          backgroundImage: 'radial-gradient(circle at 15% 50%, oklch(1 0 0 / 0.08) 0, transparent 50%), radial-gradient(circle at 85% 30%, oklch(0.85 0.13 220 / 0.15) 0, transparent 50%)',
        }} />
      ...
      <div className="mx-auto -mt-16 w-full max-w-5xl px-4 sm:-mt-20 sm:px-6">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap gap-2">
                <span className="rounded-full bg-accent/10 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent">{meta.label}</span>
                <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700">Active</span>
                <span className="rounded-full bg-secondary px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{meta.portalName}</span>
              </div>
              <h1 className="mt-1.5 font-display text-2xl font-black text-navy sm:text-3xl">{session.name}</h1>
            ...
            <div className="flex flex-wrap gap-2 sm:flex-nowrap">
              ...
              <button
                onClick={() => { signOut(); router.push('/'); toast.success('Signed out'); }}
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
          {stats.map(({ label, value, sub }) => (
            <div key={label} className="stat-card-hover rounded-xl border border-border bg-white p-4 shadow-sm">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-3 lg:items-start">
          ...
          <div className="space-y-5 lg:col-span-2">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-1.5">
                    ...
                    <span className="rounded-full bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700">Verified</span>
                  </div>
                </div>
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Workspace</label>
            ...
            <div className="rounded-2xl border border-border bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="mt-4 space-y-2">
                {[
                  { icon: Shield,  label: 'Password',          sub: 'Last changed 14 days ago',           btn: 'Change'  },
                  { icon: Phone,   label: 'Two-factor auth',   sub: 'OTP via +91 ●●●●● 4291',             btn: 'Manage'  },
                  { icon: Clock,   label: 'Active sessions',   sub: '2 devices · Mumbai, Pune',           btn: 'Review'  },
                ].map(({ icon: Icon, label, sub, btn }) => (
                  <div key={label} className="flex items-center gap-3 rounded-xl border border-border bg-secondary/30 px-4 py-3 transition hover:bg-secondary/60">
              ...
              <ul className="mt-4 space-y-0">
                {activity.map(({ time, text, dot }, i) => (
                  <li key={i} className="relative flex items-start gap-4 pb-5 last:pb-0">
                    ...
                    <span className={`relative mt-1.5 h-3.5 w-3.5 shrink-0 rounded-full ring-2 ring-white ${dot}`} />
                    <div className="flex-1">
                      <div className="text-sm font-medium text-navy">{text}</div>
                      <div className="mt-0.5 font-mono text-[11px] text-muted-foreground">{time}</div>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-navy to-navy-deep p-5 text-white shadow-sm">
              <div className="mb-1 text-[10px] font-bold uppercase tracking-widest text-white/50">Quick Actions</div>
            ...
            <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                ...
                ] as [string, keyof typeof prefs, LucideIcon][]).map(([label, key, Icon]) => (
            ...
            <div className="rounded-2xl border border-border bg-white p-5 text-center shadow-sm">
              <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Switch Portal</div>
              <p className="mt-2 text-xs text-muted-foreground">Demo any other role in one click.</p>
              <Link
                href="/admin-login"
