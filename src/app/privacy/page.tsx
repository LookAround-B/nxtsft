...
export const metadata: Metadata = {
  ...
  description: 'Learn how NxtSft.com collects, uses, shares, and protects your personal information.',
};
...
const SECTIONS = [
  ...
  },
  {
    ...
    contact: {
      ...
      responseTime: 'We aim to acknowledge complaints within 24 hours and resolve them within 30 days.',
    },
  },
  {
    id: 'changes',
        ...
        <div className="grid gap-8 lg:grid-cols-[220px_1fr]">
          ...
          <article className="space-y-10">
            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-6 py-4 text-sm text-blue-800">
              <strong>Your privacy matters.</strong> NxtSft.com does not sell your personal data to third parties. We collect only what is necessary to operate the Platform and connect you with relevant real estate opportunities.
            </div>
            ...
            {SECTIONS.map((s) => (
              <section key={s.id} id={s.id} className="scroll-mt-24">
                ...
                {'bodyAfterList' in s && s.bodyAfterList && s.bodyAfterList.map((p, i) => (
                ...
                {'contact' in s && s.contact && (
                  <div className="mt-4 rounded-xl border border-border bg-white p-5 text-sm">
                    ...
                    <div className="mt-3">
                      Email: <a href={`mailto:${s.contact.email}`} className="text-accent hover:underline">{s.contact.email}</a>
                    </div>
                    <div className="mt-2 rounded-lg bg-secondary/60 px-3 py-2 text-xs text-muted-foreground">{s.contact.responseTime}</div>
                  </div>
                )}
              </section>
            ))}
