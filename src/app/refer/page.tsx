...
import {
  Copy, Check, Share2, Gift, Users, Camera, Home, Wallet,
  ChevronDown, ChevronUp, Star, ArrowRight, Smartphone,
} from 'lucide-react';
...
/* ─── Reward tiers ────────────────────────────────────────────────── */
const REWARD_TIERS = [
  { label: 'Buyer / Tenant referral', amount: '₹500', condition: 'Per deal closed', highlight: true },
  { label: 'Owner listing referral', amount: '₹120', condition: 'Per published listing', highlight: false },
  { label: 'Board photo submission', amount: '₹50',  condition: 'Per verified & listed board', highlight: false },
  { label: 'Agent onboarding referral', amount: '₹300', condition: 'Per verified RERA agent', highlight: false },
];

/* ─── Redemption steps ────────────────────────────────────────────── */
const REDEEM_STEPS = [
  { n: '01', title: 'Earn rewards', desc: 'Complete referral actions. Rewards are credited to your NxtSft Wallet within 48 hours of verification.' },
  { n: '02', title: 'Open your Wallet', desc: 'Go to your User Portal → Wallet tab to view your available balance and transaction history.' },
  { n: '03', title: 'Choose payout method', desc: 'Redeem via UPI (recommended), bank transfer (NEFT/IMPS), or use your balance as Credits on NxtSft.com.' },
  { n: '04', title: 'Get paid instantly', desc: 'UPI payouts process within minutes. Bank transfers take 1–2 working days. Minimum redemption: ₹100.' },
];
...
/* ─── Cities ──────────────────────────────────────────────────────── */
const CITIES = ['Mumbai', 'Delhi NCR', 'Bengaluru', 'Hyderabad', 'Pune', 'Chennai', 'Ahmedabad', 'Kolkata', 'Jaipur', 'Gurugram'];
...
function CopyLinkBox({ referralCode }: { referralCode: string }) {
  ...
  const share = () => {
    if (typeof navigator !== 'undefined' && navigator.share) {
      navigator.share({ title: 'Join NxtSft.com', text: 'Find your dream home on NxtSft.com — India\'s smart real estate platform!', url: link }).catch(() => {});
    } else {
      copy();
    }
  };
  ...
  return (
    <div className="mt-6 rounded-2xl border-2 border-accent/25 bg-white p-5 shadow-sm shadow-accent/10">
      <div className="mb-2 text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Your referral link</div>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-border last:border-0">
      ...
      >
        {q}
        {open ? <ChevronUp size={16} className="mt-0.5 shrink-0 text-accent" /> : <ChevronDown size={16} className="mt-0.5 shrink-0 text-muted-foreground" />}
      </button>
...
export default function ReferPage() {
  ...
  const referralCode = session ? `${session.name.replace(/\s+/g, '').toLowerCase().slice(0, 8)}${Math.abs(session.email.charCodeAt(0) * 7).toString().slice(0, 4)}` : 'NXTSFT2026';
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <div>
              ...
              <h1 className="mt-5 font-display text-4xl font-black leading-tight text-white sm:text-5xl lg:text-6xl">
                Refer friends.<br />Earn real money.
              </h1>
              <p className="mt-5 text-base leading-relaxed text-white/70">
                Share NxtSft.com with buyers, tenants, and property owners in your network — and earn up to <strong className="text-white">₹500 per referral</strong>, paid directly to your UPI or bank account.
              </p>

              {/* Stats */}
              <div className="mt-8 flex flex-wrap gap-6">
                {[['₹500', 'per buyer deal'], ['₹120', 'per listing'], ['₹50', 'per board photo'], ['No cap', 'on earnings']].map(([v, l]) => (
                  <div key={l}>
              ...
              <div className="rounded-3xl border border-white/15 bg-white/10 p-8 backdrop-blur-md">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={u.name} className="flex items-center gap-4 rounded-2xl bg-white/8 px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-white/50">{u.city} · {u.refs} referrals</div>
                      </div>
                      <div className="font-display text-base font-bold text-emerald-300">{u.earned}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-5 flex items-center gap-2 rounded-xl bg-white/6 px-4 py-3">
                  <Star size={14} className="text-amber-400 fill-amber-400" />
                  <span className="text-xs text-white/60">Active partners earning <strong className="text-white">₹5,000–₹30,000/month</strong></span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="mb-2 text-xs font-bold uppercase tracking-widest text-accent">Earn in 3 ways</div>
        <h2 className="font-display text-3xl font-black text-navy sm:text-4xl">How would you like to earn?</h2>
          ...
          {EARN_PATHS.map((ep, i) => (
            <button
              ...
              className={`rounded-2xl border-2 p-5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${
                activeEarn === i ? `${ep.bgLight} -translate-y-0.5 shadow-md` : 'border-border bg-white'
              }`}
            >
              <div className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${ep.tagColor}`}>
                <ep.icon size={11} />
                {ep.tag}
              </div>
        ...
        <div className={`mt-6 rounded-3xl border-2 p-8 transition-all ${EARN_PATHS[activeEarn].bgLight}`}>
          <div className="grid gap-8 md:grid-cols-2">
            <div>
              <div className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${EARN_PATHS[activeEarn].tagColor}`}>
                <ActiveEarnIcon size={13} />
                {EARN_PATHS[activeEarn].tag}
              </div>
              <div className="mt-4 font-display text-4xl font-black text-navy">{EARN_PATHS[activeEarn].reward} <span className="text-lg font-semibold text-muted-foreground">{EARN_PATHS[activeEarn].rewardNote}</span></div>
              <p className="mt-4 text-sm leading-relaxed text-foreground/70">{EARN_PATHS[activeEarn].desc}</p>
            </div>
            <div>
              <div className="mb-4 text-[11px] font-bold uppercase tracking-widest text-muted-foreground">How it works</div>
              <ol className="space-y-4">
                {EARN_PATHS[activeEarn].steps.map((step, i) => (
                  <li key={i} className="flex gap-4">
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br text-white text-xs font-black ${EARN_PATHS[activeEarn].color}`}>
                      {i + 1}
                    </div>
      ...
      <section className="border-y border-border bg-white">
        <div className="mx-auto max-w-6xl px-6 py-16">
          <div className="mb-2 text-xs font-bold uppercase tracking-widest text-accent">Payout schedule</div>
          <h2 className="font-display text-3xl font-black text-navy sm:text-4xl">What you earn</h2>
          <div className="mt-8 overflow-hidden rounded-2xl border border-border">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/40">
                  <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Referral type</th>
                  <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Reward</th>
                  <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Condition</th>
                </tr>
              </thead>
              <tbody>
                {REWARD_TIERS.map((r, i) => (
                  <tr key={r.label} className={`border-b border-border last:border-0 transition hover:bg-secondary/30 ${r.highlight ? 'bg-accent/4' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      <span className={`font-display text-xl font-black ${r.highlight ? 'text-accent' : 'text-navy'}`}>{r.amount}</span>
                    </td>
          ...
          <p className="mt-4 text-xs text-muted-foreground">Rewards are credited within 48 hours of verification. Minimum redemption ₹100. Valid for properties in India only.</p>
        </div>
      </section>

      {/* ── How to redeem ─────────────────────────────────────────── */}
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="mb-2 text-xs font-bold uppercase tracking-widest text-accent">Redemption</div>
        <h2 className="font-display text-3xl font-black text-navy sm:text-4xl">Get paid in 4 steps</h2>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {[
            ...
            { label: 'NxtSft Credits', note: 'Use to search & unlock listings', icon: Gift, highlight: false },
          ].map((m) => (
            <div
              key={m.label}
              className={`flex items-center gap-4 rounded-2xl border p-5 ${m.highlight ? 'border-accent/30 bg-accent/6' : 'border-border bg-white'}`}
            >
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${m.highlight ? 'bg-accent text-white' : 'bg-secondary text-foreground/60'}`}>
                <m.icon size={18} />
              </div>
              <div>
                <div className="font-semibold text-sm text-navy">{m.label} {m.highlight && <span className="ml-1.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-bold uppercase text-emerald-700">Recommended</span>}</div>
      ...
      <section className="border-y border-border bg-white">
        <div className="mx-auto max-w-6xl px-6 py-14">
          <div className="mb-2 text-xs font-bold uppercase tracking-widest text-accent">Available in</div>
          <h2 className="font-display text-2xl font-black text-navy">Board photo submissions — active cities</h2>
          <div className="mt-6 flex flex-wrap gap-2">
            {CITIES.map((c) => (
              <span key={c} className="rounded-full border border-border bg-secondary/40 px-4 py-2 text-sm font-medium text-foreground/70">
                {c}
              </span>
            ))}
            <span className="rounded-full border border-dashed border-border px-4 py-2 text-sm font-medium text-muted-foreground">More cities coming soon</span>
          </div>
          <p className="mt-5 text-xs text-muted-foreground">Buyer/owner referrals and listing referrals are available for all properties across India.</p>
        </div>
      </section>

      {/* ── FAQ ───────────────────────────────────────────────────── */}
      <section className="mx-auto max-w-3xl px-6 py-16">
        <div className="mb-2 text-xs font-bold uppercase tracking-widest text-accent">Common questions</div>
        <h2 className="font-display text-3xl font-black text-navy sm:text-4xl">Frequently asked questions</h2>
      ...
      <section className="border-t border-border bg-secondary/30">
        <div className="mx-auto max-w-4xl px-6 py-10">
          <div className="mb-3 text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Programme Terms &amp; Conditions</div>
      ...
      {!session && (
        ...
        >
          ...
          <div className="relative z-10 mx-auto max-w-3xl px-6 py-20 text-center">
            <div className="font-display text-3xl font-black text-white sm:text-4xl">Start earning today.<br />It&apos;s completely free.</div>
            <p className="mt-4 text-white/70">Create your account in 60 seconds and start sharing your referral link.</p>
