...
function FAQ({ faqs, title }: { faqs: string[][]; title?: string }) {
  ...
  return (
    <section className="mx-auto max-w-4xl px-6 py-14">
      <div className="mb-2 text-xs font-bold uppercase tracking-widest text-accent text-center">Common questions</div>
      <h2 className="text-center font-display text-2xl font-black text-navy sm:text-3xl">{title ?? 'Frequently asked questions'}</h2>
      <p className="mt-3 text-center text-sm text-muted-foreground">Everything you need to know before you buy.</p>
      <div className="mt-8 divide-y divide-border rounded-2xl border border-border bg-white overflow-hidden shadow-sm">
        {faqs.map(([q, a], i) => (
          <div key={q}>
            ...
            >
              <span className="font-display text-sm font-bold text-navy pr-2 leading-snug">{q}</span>
...
const CHOOSER_FLOWS: Record<ChooserVariant, {
  ...
  'owner-rent': {
    ...
    recommend: ([props, speed, help]) => {
      if (props === '4+' || help === 'full') return { planId: 'rent-platinum', name: 'Rent Monthly Platinum', why: 'Covers 15 listings with social ad engine and automated lead routing — ideal for agents managing multiple properties.' };
      if (props === '2-3') return { planId: 'rent-gold', name: 'Rent Monthly Gold', why: 'Lists up to 3 properties with homepage rotation and 75 verified leads — perfect for small portfolios.' };
      if (speed === 'urgent') return { planId: 'rent-weekly', name: 'Rent Weekly Booster', why: 'Get maximum visibility for 7 days with top placement — best when you need a tenant fast.' };
      return { planId: 'rent-silver', name: 'Rent Monthly Silver', why: 'Single listing, 30 days, 30 verified leads with WhatsApp alerts — the right fit for most individual landlords.' };
    },
  },
  'owner-sell': {
    steps: [
    ...
    recommend: ([props, mkt, crm]) => {
      if (crm === 'yes' || props === '7+') return { planId: 'sell-builder', name: 'Sell Monthly Builder Pro', why: 'CRM integration, Google + Meta campaigns, 15 listings and 400 leads — built for serious developers.' };
      if (mkt === 'full') return { planId: 'sell-platinum', name: 'Sell Monthly Platinum', why: 'Homepage rotation, 7 listings, 200 leads, and dedicated lead filters — for premium agencies.' };
      if (props === '2-3' || mkt === 'boosted') return { planId: 'sell-gold', name: 'Sell Monthly Gold', why: '3 listings, Facebook Ad Boost, top placement and 120 verified buyer leads — the sweet spot for brokers.' };
      return { planId: 'sell-silver', name: 'Sell Monthly Silver', why: 'Single property, 50 verified buyer leads and featured tag — the right start for a direct homeowner.' };
    },
  },
  seeker: {
    steps: [
    ...
    recommend: ([count, rm, when]) => {
      if (rm === 'yes' && count === '12+') return { planId: 'executive', name: 'Executive Premium', why: '50 unlocks, a dedicated RM, 10 managed site visits, and end-to-end closing support — best for buyers seeking a complete service.' };
      if (rm === 'yes') return { planId: 'pro', name: 'Pro Assisted', why: '24 unlocks with a dedicated Relationship Manager and up to 5 walkthroughs — great if you want guided support during your search.' };
      if (count === '12+') return { planId: 'hunter', name: 'Active Hunter', why: '12 contact unlocks for 30 days — ideal for an aggressive multi-property search without needing a RM.' };
...
function PlanChooser({ variant, onScrollToPlans }: { variant: ChooserVariant; onScrollToPlans: (id: string) => void }) {
    ...
    <section className="mx-auto max-w-6xl px-5 pb-6 sm:px-6">
      <div className="rounded-3xl border-2 border-accent/20 bg-gradient-to-br from-accent/5 to-transparent p-8">
        <div className="flex items-center gap-2.5 mb-1">
          ...
          <div className="text-xs font-bold uppercase tracking-widest text-accent">Plan Customisation</div>
        </div>
        <h3 className="font-display text-xl font-black text-navy">Not sure which plan to pick?</h3>
        <p className="mt-1 text-sm text-muted-foreground">Answer 3 quick questions and we'll recommend the best plan for your situation.</p>
        ...
        {!result ? (
          <div className="mt-7">
            ...
            <div className="mb-5 flex items-center gap-2">
              {flow.steps.map((_, i) => (
                <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${i <= step ? 'bg-accent' : 'bg-border'}`} />
              ))}
            </div>
            <div className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-3">
              Step {step + 1} of {flow.steps.length}
            </div>
            <div className="font-display text-base font-bold text-navy mb-5">{flow.steps[step].q}</div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:flex lg:flex-wrap">
              {flow.steps[step].options.map((opt) => (
                ...
                >
                  <span className="font-semibold text-sm text-navy group-hover:text-accent">{opt.label}</span>
                  {opt.sub && <span className="mt-0.5 text-xs text-muted-foreground">{opt.sub}</span>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mt-7">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
              ...
              <div className="flex-1">
                <div className="text-[11px] font-bold uppercase tracking-widest text-accent mb-1">We recommend</div>
...
export default function PricingPage() {
  ...
  const scrollToPlan = (planId: string) => {
    ...
    el?.animate([{ boxShadow: '0 0 0 4px oklch(0.65 0.22 27 / 0.4)' }, { boxShadow: '0 0 0 0px oklch(0.65 0.22 27 / 0)' }], { duration: 1200, easing: 'ease-out' });
  };
      ...
      {activeTab === 0 && (
        <>
          ...
          <section className="mx-auto max-w-6xl px-5 pt-10 pb-6 sm:px-6">
            <div className={`grid grid-cols-1 items-stretch gap-6 pt-5 sm:grid-cols-2 ${ownerMode === 'renting' ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>
              {ownerPlans.map((plan) => (
