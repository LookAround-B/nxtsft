...
import {
  ...
  Calculator, FileCheck, Settings2, CreditCard, Eye,
  Bell, Clock, X, Plus, Pause, Play, Download, ChevronDown,
} from 'lucide-react';
...
function renderTab(h: string, userEmail: string) {
  switch (h) {
    ...
    case 'viewed':  return <RecentlyViewed />;
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Credits Remaining"      value="4"   sub="Standard Pack" accent="text-emerald-600" />
        ...
        {myViews.length === 0 ? (
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {myViews.slice(0, 3).map((v) => {
              ...
              return (
                <Link key={v.id} href={`/properties/${v.propertyId}`} className="group flex items-center gap-3 rounded-lg border border-border bg-white p-3 hover:border-accent/40 transition-colors">
                  {prop && (
                    <img src={prop.image} alt="" className="h-12 w-16 flex-shrink-0 rounded-md object-cover" />
                  )}
                  <div className="min-w-0">
                    <div className="truncate text-xs font-semibold text-navy group-hover:text-accent">{v.propertyTitle}</div>
                    <div className="mt-0.5 flex items-center gap-1 text-[10px] text-muted-foreground">
                      <Clock size={9} />{fmtDur(v.durationSec)} · {v.city}
                    </div>
                    <div className="mt-0.5 font-mono text-[9px] text-muted-foreground/70">{v.ts.split(' ')[0]}</div>
                  </div>
                </Link>
              );
            })}
      ...
      {/* Recommended for you */}
      <Section title="Recommended for you" action={<Link href="/properties" className="text-xs font-semibold text-accent">Browse more →</Link>}>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {featuredProps.map((p) => (
            <Link key={p.id} href={`/properties/${p.id}`} className="group overflow-hidden rounded-lg border border-border bg-white">
              ...
              <div className="p-3">
                <div className="text-xs text-muted-foreground">{p.locality} · {p.bhk}</div>
      ...
      <Section title="Upcoming site visits">
        <div className="grid gap-3 sm:grid-cols-2">
          {dummyVisits.map((v) => (
            <div key={v.property} className="flex items-start gap-3 rounded-lg border border-border bg-secondary/30 p-4">
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        {(filterBhk || filterCity) && (
          <button onClick={() => { setFilterBhk(''); setFilterCity(''); }} className="flex items-center gap-1 text-xs text-accent">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-secondary/20 py-16 text-center">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No saved properties match your filters.</p>
          <button onClick={() => { setFilterBhk(''); setFilterCity(''); }} className="mt-2 text-xs text-accent hover:underline">Clear filters</button>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.map((p) => (
            <div key={p.id} className="overflow-hidden rounded-lg border border-border bg-white">
              ...
              <div className="p-3">
                <div className="text-xs text-muted-foreground">{p.locality} · {p.bhk} · {p.city}</div>
                <div className="text-sm font-semibold text-navy">{p.title}</div>
                <div className="mt-2 flex items-center justify-between">
                  <span className="font-display text-base font-bold text-accent">{p.priceLabel}</span>
                  <button
                    onClick={() => {
        ...
        <div className="overflow-hidden rounded-lg border border-border">
          ...
          <div className="p-4">
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={() => toast.success('Listing boosted')} className="rounded-md bg-accent px-3 py-1.5 text-xs font-semibold text-accent-foreground">Boost</button>
              <button onClick={() => toast('Edit mode')} className="rounded-md border border-border px-3 py-1.5 text-xs font-semibold">Edit</button>
            </div>
          </div>
        </div>
      </Section>
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Contacts Unlocked"  value={String(unlocked)}       sub={`of ${history.length} views`} />
      ...
      <div className="mt-4 mb-2 flex items-center justify-between rounded-lg border border-border bg-secondary/30 px-4 py-3">
        ...
        <button
          onClick={() => { setHistory([]); toast('History cleared'); }}
            ...
            {history.map((v) => {
              ...
              return (
                <div key={v.id} className="flex items-center gap-4 py-4">
                  {prop && (
                    <img src={prop.image} alt={prop.title} className="h-14 w-20 flex-shrink-0 rounded-lg object-cover" />
                  )}
                  ...
                  <div className="flex flex-shrink-0 flex-col items-end gap-2">
                    ...
                    <Link href={`/properties/${v.propertyId}`} className="text-xs font-semibold text-accent hover:underline">
                      View →
                    </Link>
                  </div>
                </div>
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Tokens Remaining"  value={String(mySubscription.creditsLeft)}          sub={`of ${mySubscription.creditsTotal} total`} accent={mySubscription.creditsLeft === 0 ? 'text-accent' : 'text-emerald-600'} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowTopUp(false)} className="rounded-md p-1 hover:bg-secondary"><X size={16} /></button>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {seekerPlans.slice(0, 3).map((plan) => (
                <button
                  key={plan.id}
                  onClick={() => { setShowTopUp(false); toast.success(`${plan.name} purchased!`); }}
                ...
                >
                  ...
                  <div className="mt-0.5 text-xs font-semibold text-accent">{plan.credits} credits</div>
                  <div className="mt-1 text-xs text-muted-foreground">{plan.name}</div>
                  <div className="mt-0.5 text-[10px] text-muted-foreground">Valid {plan.validity}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-5">
            {creditTimeline.map((entry, i) => (
              <div key={i} className="relative flex items-start gap-3">
                {/* Dot */}
                <div className={`absolute -left-[15px] mt-0.5 h-3 w-3 rounded-full border-2 border-white ${entry.tokens > 0 ? 'bg-emerald-500' : 'bg-accent'}`} />
                <div>
                  <div className="text-sm font-medium text-navy">{entry.action}</div>
                  <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                    <span className="font-mono">{entry.date}</span>
                    <span className={`font-bold ${entry.tokens > 0 ? 'text-emerald-600' : 'text-accent'}`}>
                      {entry.tokens > 0 ? `+${entry.tokens}` : `${entry.tokens}`} token{Math.abs(entry.tokens) !== 1 ? 's' : ''}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {myLedger.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {myLedger.map((entry) => {
              ...
              const badgeClass = isCredit ? 'bg-emerald-100 text-emerald-700' : isRefund ? 'bg-blue-100 text-blue-700' : 'bg-accent/10 text-accent';
      ...
      <Section title="Personal Information">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">City</label>
            <select value={city} onChange={(e) => setCity(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm">
              {['Mumbai', 'Bengaluru', 'Pune', 'Delhi', 'Hyderabad', 'Chennai'].map((c) => <option key={c}>{c}</option>)}
            </select>
          </div>
        </div>
      ...
      <Section title="Property Preferences">
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <select value={budget} onChange={(e) => setBudget(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm">
              {['Under ₹50L', '₹50L – ₹1.5Cr', '₹1.5Cr – ₹3Cr', '₹3Cr – ₹6Cr', '₹6Cr+'].map((b) => <option key={b}>{b}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Preferred Type</label>
            <select value={propType} onChange={(e) => setPropType(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm">
              {['Apartment', 'Villa', 'Studio', 'Bungalow', 'Commercial'].map((t) => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Buy Timeline</label>
            <select value={timeline} onChange={(e) => setTimeline(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm">
              {['Immediately', '1–3 months', '3–6 months', '6–12 months', 'Just exploring'].map((t) => <option key={t}>{t}</option>)}
            </select>
          </div>
        </div>
        <button onClick={() => toast.success('Preferences saved')} className="mt-4 rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground">Save Preferences</button>
      </Section>
      ...
      <Section title="Change Password">
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <input type="password" value={curPwd} onChange={(e) => setCurPwd(e.target.value)} placeholder="••••••••" className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">New Password</label>
            <input type="password" value={newPwd} onChange={(e) => setNewPwd(e.target.value)} placeholder="••••••••" className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Confirm Password</label>
            <input type="password" value={confPwd} onChange={(e) => setConfPwd(e.target.value)} placeholder="••••••••" className="mt-1 w-full rounded-md border border-border bg-white px-3 py-2 text-sm" />
          </div>
        </div>
        <button
          onClick={() => {
            if (!curPwd || !newPwd || !confPwd) { toast.error('Fill all fields'); return; }
            if (newPwd !== confPwd) { toast.error('Passwords do not match'); return; }
            toast.success('Password updated'); setCurPwd(''); setNewPwd(''); setConfPwd('');
          }}
    ...
    <>
      ...
      >
        {alerts.length === 0 ? (
          ...
          <div className="space-y-3">
            {alerts.map((a) => (
              <div key={a.id} className={`rounded-xl border p-4 transition-colors ${a.active ? 'border-border bg-white' : 'border-border bg-secondary/20'}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-navy">{a.name}</span>
                      {!a.active && (
                        <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">Paused</span>
                      )}
                    </div>
                    {/* Criteria */}
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      <span className="rounded-md bg-secondary px-2 py-0.5 text-[10px] font-medium text-muted-foreground">{a.location}</span>
                      <span className="rounded-md bg-secondary px-2 py-0.5 text-[10px] font-medium text-muted-foreground">{a.bhk}</span>
                      <span className="rounded-md bg-secondary px-2 py-0.5 text-[10px] font-medium text-muted-foreground">{a.budget}</span>
                    </div>
                    <div className="mt-2 text-xs text-muted-foreground">
                      <span className="font-semibold text-accent">{a.lastMatchCount}</span> properties matched in the last check
                    </div>
                  </div>
                  {/* Actions */}
  ...
  return (
    <>
      ...
      <Section title="Upcoming">
        <div className="space-y-3">
          {[
            { property: 'Skyline Residences', when: 'Sat, 14 Jun · 11:00 AM', agent: 'Priya Sharma' },
            ...
            <div key={v.property} className="rounded-lg border border-border bg-secondary/40 p-4">
              ...
              <div className="text-xs text-muted-foreground">{v.when} · with {v.agent}</div>
              <div className="mt-3 flex gap-2">
                <button onClick={() => toast.success('Visit rescheduled')} className="rounded-md bg-mid-blue px-3 py-1 text-xs font-semibold text-white">Reschedule</button>
                <button onClick={() => toast.error('Visit cancelled')}     className="rounded-md border border-border px-3 py-1 text-xs font-semibold">Cancel</button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <>
      ...
      <Section title="Status">
        {([['Aadhaar', 'Verified', 'success'], ['PAN Card', 'Verified', 'success'], ['Income Proof', 'Pending', 'warm']] as const).map(([d, s, t]) => (
          <div key={d} className="flex items-center justify-between border-b border-border py-3 last:border-0">
            <span className="text-sm">{d}</span>
            <div className="flex items-center gap-2">
              <Badge tone={t}>{s}</Badge>
              {s === 'Pending' && (
                <button onClick={() => toast.success('Document uploaded')} className="rounded-md bg-accent px-3 py-1 text-xs font-semibold text-accent-foreground">Upload</button>
              )}
            </div>
          </div>
        ))}
