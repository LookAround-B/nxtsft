...
export default function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (!/^\d{10}$/.test(form.phone.replace(/\s/g, ''))) errs.phone = 'Enter a 10-digit phone number';
        ...
        <div className="hidden animate-fade-up flex-col justify-between rounded-3xl bg-gradient-to-br from-navy via-navy-deep to-accent p-10 text-white lg:flex">
          <div>
            ...
            <h1 className="mt-6 font-display text-4xl font-black leading-tight">
              Your perfect home<br />is one step away.<br />
              <span className="text-white/55">Sign up. Explore. Own.</span>
            </h1>
            <p className="mt-6 max-w-md text-sm leading-relaxed text-white/65">
              Create a free account to save properties, schedule site visits, unlock owner contacts and get AI-matched listings straight to your inbox.
            </p>
        ...
        <div className="mx-auto w-full max-w-lg animate-fade-up delay-75">
          {session && (
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-secondary/60 px-4 py-3 text-sm">
              <span className="text-foreground/80">Already signed in as <strong className="text-navy">{session.name}</strong></span>
              <button onClick={() => signOut()} className="rounded-lg border border-border bg-white px-3 py-1.5 text-xs font-semibold text-navy transition hover:bg-secondary">
                Sign out
              </button>
            </div>
          )}
          ...
          <p className="mt-2 text-sm text-muted-foreground">Join 1 lakh+ buyers on India&apos;s smartest real estate platform.</p>

          <form onSubmit={submit} noValidate className="mt-6 space-y-4 rounded-2xl border border-border bg-white p-6 shadow-sm">
