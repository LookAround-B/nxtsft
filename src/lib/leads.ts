export type LeadAction = 'Schedule Visit' | 'Request Callback' | 'Unlock Contact' | 'WhatsApp' | 'Get Price';
...
export function captureLead(
  ...
  try { localStorage.setItem(LEADS_KEY, JSON.stringify(existing)); } catch { /* ignore */ }
}
...
export function assignLead(id: string, memberName: string): void {
  ...
  try { localStorage.setItem(LEADS_KEY, JSON.stringify(list)); } catch { /* ignore */ }
}
